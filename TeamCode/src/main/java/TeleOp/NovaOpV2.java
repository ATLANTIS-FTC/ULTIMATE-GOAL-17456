...
@Config
@TeleOp(group = "drive")
public class NovaOpV2 extends LinearOpMode {
    ...
    private Vector2d targetPosition = new Vector2d(70, 36);
...
//                            .build();
                    ...
                    while (drive.shooterOne.getVelocity() > -1320 && opModeIsActive()) {
                        telemetry.addData("Velo:", drive.shooterOne.getVelocity());
                        telemetry.update();
                        if (gamepad1.x) {
                            currentMode = Mode.NORMAL_CONTROL;
...
//                    shooter.setPower(powerPwr, drive);
            ...
            if (spindex) {
                ...
                if (counter == 0) {
                    ...
                    telemetry.update();
                    spindexer.ringPositionOne(drive);
                    counter++;
                    spindex = false;
                } else if (counter == 1) {
                    telemetry.clear();
                    telemetry.addLine("ayan");
                    telemetry.update();
                    spindexer.ringPositionFive(drive);
                    counter++;
                    spindex = false;
                } else {
                    telemetry.clear();
                    ...
                    telemetry.update();
                    spindexer.ringPositionTwo(drive);
//                    telemetry.clear();
